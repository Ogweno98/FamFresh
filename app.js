/* app.js — FamFresh main frontend logic
   Requires firebase-init.js to be included first
*/

// ----- Config placeholders -----
const CHECKOUT_SERVER_URL = "YOUR_CHECKOUT_SERVER_URL"; // e.g. https://us-central1-.../createCheckoutSession

// ----- Short wrappers for Firebase compat APIs -----
const auth = firebase.auth();
const db = firebase.firestore();
const storage = firebase.storage();

// ----- auth helpers exposed to pages -----
window.appAuth = {
...
window.ui = {
  ...
  async updateCartCountUI(){ const cart = JSON.parse(localStorage.getItem('cart')||'[]'); const el=document.getElementById('cartCount'); if(el) el.textContent = cart.length; }
};

// ----- Chatbot bridge (frontend) - appChat is implemented in chatbot.js but we provide fallback here -----
window.appChat = window.appChat || {
  addMessage(role, text){ const el = document.querySelector('#chatMessages'); if(!el) return; el.innerHTML += `<div class="${role==='bot'?'text-sm text-gray-800':'text-sm text-green-700'}"><strong>${role==='bot'?'FamFresh':'You'}:</strong> ${text}</div>`; el.scrollTop = el.scrollHeight; },
...
// update buyerName on auth
appAuth && appAuth.onAuthStateChanged && appAuth.onAuthStateChanged(async user => {
  if(user) {
    try{
